...
for i in Subj:
    files.extend([f for f in listdir(path+i) if isfile(join(path+i+'\\',f))])
...
def get_fitness(indvid,stim):
    p=pd.Series()
    for i in range(8):
        correlaton = np.corrcoef(indvid,ref.T[i])
...
def compCorr(signal,ref):
    result=0
    for i in range(0,5):
        correlaton14 = abs(np.corrcoef(signal[0],ref.T[i]))
        correlaton8 = abs(np.corrcoef(signal[1],ref.T[i]))
        print("%.3f, %.3f, %.3f" %(correlaton14[0][1], correlaton8[0][1],
                                   correlaton14[0][1] -correlaton8[0][1]))
        if correlaton14[0][1]>correlaton8[0][1]:
            result+=1
    for i in range(5,10):
        correlaton14 = abs(np.corrcoef(signal[0],ref.T[i]))
        correlaton8 = abs(np.corrcoef(signal[1],ref.T[i]))
...
def classify():
    print("Skuteczność klasyfikacji otrzymanych:", compCorr(bestIndvid.T,ref))
    meanSig=pd.concat([pd.DataFrame(ref[0:3].mean()).T,
                       pd.DataFrame(ref[5:8].mean()).T,
                       pd.DataFrame(ref[10:13].mean()).T,
                       ], ignore_index=True)
    print("Skuteczność klasyfikacji uśrednionych:", compCorr(meanSig.T,ref))

    perfectSig=pd.concat([perfectSin14.T, perfectSin8.T], ignore_index=True)
    print("Skuteczność klasyfikacji idealnych:", compCorr(perfectSig.T,ref))
...
for fileName in files:
    ...
    ref=ref.append(data, ignore_index=True)
...
for s in stim:
    if s==14:
        x=pd.DataFrame(ref[0:3].mean())
    elif s==28:
        x=pd.DataFrame(ref[5:8].mean())
    elif s=8:
        x=pd.DataFrame(ref[10:13].mean())
    ...
    for i in range(numberOfSteps):
        ...
        #print('%.2f percent' % (i/numberOfSteps * 100)),
