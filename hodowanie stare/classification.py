...
def classification(signal1GA,signal2GA,signal1,signal2,set,clss):
    results=[]
    t=np.linspace(0,int(len(signal1GA)/256),len(signal1GA))
    ref14=np.sin(2*np.pi*14*t)
    ref8=np.sin(2*np.pi*8*t)
    for i in range(4):
        corr11=computeCorr(set[:,i],signal1GA)**2
        corr21=computeCorr(set[:,i],signal2GA)**2
        corr12=computeCorr(set[:,i],signal1)**2
        corr22=computeCorr(set[:,i],signal2)**2
        corr13=computeCorr(signal1GA,signal1)**2
        corr23=computeCorr(signal1GA,signal2)**2
