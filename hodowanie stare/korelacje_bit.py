...
def decode_indv(indv):
    ...
    for i in range(0,len(indv),n_bits):
        indv_float=bin2dec(indv[i:i+n_bits])/100
        if i%(n_bits*2)==0:
...
def check_fitness(population):
    ...
    for i in range(n_pop):
        ...
        wynik=sqrt(2)-sqrt(pow(corr8[0],2)+pow(corr14[0]-1,2))
        result.append(wynik)
        if max_wynik<wynik:
            punkty=(corr8[0],corr14[0])
    dupa.append(punkty)
    return result
result=[]
...
for i in range(len(dupa)):
    plt.plot(dupa[i][0],dupa[i][1],color=(i/len(dupa),0,0), marker='o')
plt.show()
