...
for j=1:1:10
    for i=1:3:18
        ...
        %czy na pewno dobrze sciskane sa macierze?
        dataIn=[dataIn14,dataIn28,dataIn8];
        [populationOut(:,i),fitness(:,i)]=generation(dataIn,populationSize, numberOfMutations, numberOfSteps, dataLength);
        
        dataIn=[dataIn28,dataIn14,dataIn8];
        [populationOut(:,i+1),fitness(:,i+1)]=generation(dataIn,populationSize, numberOfMutations, numberOfSteps, dataLength);
        
        dataIn=[dataIn8,dataIn14,dataIn28];
        [populationOut(:,i+2),fitness(:,i+2)]=generation(dataIn,populationSize, numberOfMutations, numberOfSteps, dataLength);
    end
    plot(populationOut)
